...
#   NUM_PROCESSES=4 BATCH_SIZE=8 bash examples/LIBERO/train_files/run_libero_qwenfast_train.sh
################################################################################
FRAMEWORK_NAME="${FRAMEWORK_NAME:-QwenFast}"
BASE_VLM="${BASE_VLM:-/mnt/hdfs/data/dumengfei/checkpoints/Qwen3-VL-4B-Instruct-Action}"
CONFIG_YAML="${CONFIG_YAML:-./examples/LIBERO/train_files/starvla_cotrain_libero.yaml}"
LIBERO_DATA_ROOT="${LIBERO_DATA_ROOT:-/mnt/hdfs/data/dumengfei/data/playground/Datasets}"
DATA_MIX="${DATA_MIX:-libero_all}"
RUN_ROOT_DIR="${RUN_ROOT_DIR:-/mnt/hdfs/data/dumengfei/data/playground/Checkpoints}"
RUN_ID="${RUN_ID:-qwenfast_libero_all_qwen3vl4b_action}"
...
WANDB_PROJECT="${WANDB_PROJECT:-starVLA_Libero_QwenFast}"
...
echo "  data_root: ${RESOLVED_LIBERO_DATA_ROOT}"
...
accelerate launch \
  ...
  --datasets.vla_data.data_root_dir "${RESOLVED_LIBERO_DATA_ROOT}" \
