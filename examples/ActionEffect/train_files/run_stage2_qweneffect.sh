#!/usr/bin/env bash
# Stage 2: train QwenEffect (VLM predicts <effect_*> then <robot_action_*>).
# Prerequisites:
#   1) Stage 1 done -> effect_tokenizer.pt
#   2) add_effect_tokens.py done -> <Qwen3-VL-4B-Instruct-ActionEffect> checkpoint
set -euo pipefail
...
RUN_ID="${RUN_ID:-qweneffect_libero_all}"
...
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-39848}"
