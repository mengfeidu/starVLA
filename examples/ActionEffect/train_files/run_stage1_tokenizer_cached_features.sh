#!/usr/bin/env bash
# Stage 1: train Effect Tokenizer from cached Qwen-ViT features.
set -euo pipefail
...
FEATURE_CACHE_DIR="${FEATURE_CACHE_DIR:-/mnt/hdfs/data/dumengfei/data/playground/Features/qwen_vit_libero_all_plus_90_effect}"
RUN_ROOT_DIR="${RUN_ROOT_DIR:-/mnt/hdfs/data/dumengfei/data/playground/Checkpoints}"
RUN_ID="${RUN_ID:-action_effect_tok_vq_cached_qwen256}"
...
FEATURE_SHARD_CACHE_SIZE="${FEATURE_SHARD_CACHE_SIZE:-2}"
...
mkdir -p "${OUTPUT_DIR}"
echo "Stage1 tokenizer from cached Qwen features | data=${RESOLVED_LIBERO_DATA_ROOT} mix=${DATA_MIX} features=${FEATURE_CACHE_DIR} out=${OUTPUT_DIR}"
